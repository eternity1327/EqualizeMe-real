 ...
 *
 * Ported from backend/hearing_preservation.py, wording unchanged.
 *
 * This is the last section of the results page and the least important to
 * the rest of the system. It reads the target; it never writes to it.
 * Nothing here can reach the target curve, the region analysis, or the IEM
 * ranking — hp_build() takes a profile and returns text, and that is the
 * whole of its contract with the application.
 *
 * Rules this is written to, in order of importance:
 *
 *   1. Never diagnose. A preference for less treble is a preference for
 *      less treble. It is not evidence of hearing loss, tinnitus,
 *      sensitivity, or anything else about the person's ears, and nothing
 *      here may imply otherwise.
 *
 *   2. Never criticise the preference. There is no wrong answer to give in
 *      a listening test. The guidance is about playback level, which is a
 *      separate thing from the shape of the curve.
 *
 *   3. Say what is actually known. The one numeric reference is the
 *      WHO/ITU safe-listening standard, quoted as what it is.
 *
 * Every message has the same shape: here is what you like, and here is how
 * to enjoy it without raising your exposure more than you need to.
 */

require_once __DIR__ . "/preference_profile.php";

const HP_ELEVATED = ["slight_up", "up", "strong_up"];
const HP_REDUCED = ["slight_down", "down", "strong_down"];

// Only flag bass once the preference is beyond "slightly" — a small lift
// is not worth a paragraph of its own.
const HP_NOTABLE = ["up", "strong_up", "down", "strong_down"];


function hp_find($regions, $band) {
...
function hp_treble_note($region) {
    $level = $region["level"];

    if (in_array($level, HP_ELEVATED, true)) {
        return [
            "band" => "treble_gain",
    ...
    if (in_array($level, HP_REDUCED, true)) {
        return [
            "band" => "treble_gain",
            "title" => "About your treble preference",
            "body" =>
...
function hp_bass_note($region) {
    if (!in_array($region["level"], HP_NOTABLE, true) || $region["direction"] !== "up") {
        return null;
    }
...
// so no item reads as a response to anything in the person's profile.
function hp_tips() {
    return [
 ...
 */
function hp_build($profile) {
    if (!$profile) {
        return null;
    }

    $regions = $profile["regions"] ?? pp_analyse_regions($profile);

    $notes = [];

    $bass = hp_find($regions, "bass_gain");
    if ($bass) {
        $note = hp_bass_note($bass);
        if ($note) {
            $notes[] = $note;
        }
    }

    $treble = hp_find($regions, "treble_gain");
    if ($treble) {
        $notes[] = hp_treble_note($treble);
    }

    $tips = [];
    foreach (hp_tips() as $tip) {
        $tips[] = ["title" => $tip[0], "body" => $tip[1]];
    }
    ...
    return [
        ...
        "notes" => $notes,
        "tips" => $tips,
        "standard_note" => HP_STANDARD_NOTE,
        "disclaimer" => HP_DISCLAIMER,
    ];
}
